...
const ShowCustomerListBtn = ({ children, className, active, onClick }) => (
	<button
		...
		className={twMerge(
			...
			} text-sm not-italic font-semibold leading-5 tracking-[0.00938rem] flex items-center gap-1 ${className}`
		)}
	>
		{children}
	</button>
...
const CustomerListContainer = ({ children, className }) => (
	<ul
		className={twMerge(
			`shadow-[0px_0px_4px_0px_rgba(0,0,0,0.25)] max-w-full rounded-sm bg-white divide-y-[0.1px] divide-gray-300 max-h-[22rem] overflow-x-hidden overflow-y-auto ${className}`
		)}
	>
		{children}
	</ul>
...
const CustomerItem = ({ customer, onClick }) => {
	...
	return (
		...
		>
			<p
				onClick={handleSet}
				className="text-[#2B2E72] text-sm not-italic font-medium leading-5 truncate"
			>
				{customer?.first_name} {customer?.last_name}
			</p>
			<p
				onClick={handleSet}
				className="text-[#706E6E] text-xs not-italic font-normal leading-5 truncate"
			>
				{customer.email}
			</p>
		</button>
...
const SelectCustomerAndTicketsDropdown = ({ className }) => {
	...
	return (
		<motion.div
			...
			className={twMerge(
				`inline-flex flex-col items-start shadow-[0px_0px_4px_0px_rgba(0,0,0,0.25)] pt-1 pb-4 px-3 py-3 rounded-xl border-[0.5px] border-solid border-black bg-white translate-x-2 ${className}`
			)}
		>
				...
				<CustomerListContainer className="absolute w-full top-0 left-0 z-10">
					{showCustomerList &&
						filteredCustomers.map((customer) => (
							<CustomerItem key={customer.id} customer={customer} onClick={() => {console.log("juyv")}}/>
					))}
				</CustomerListContainer>
				<ShowCustomerListBtn className="mt-4" active={validCustomerSelected}>
					Create Ticket <PlusIcon fill={"#2B2E72"} />
				</ShowCustomerListBtn>
			</div>
		</motion.div>
	);
