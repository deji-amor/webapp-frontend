...
const HeaderContent = ({title, titleSize, desWidth, titleColor, iconColor, iconSize, description, icon, size, align, color, width, style, padding}) => {
  return (
    <HeaderContentWrapper padding={padding} align={align}>
      <Icon icon={icon} iconSize={iconSize} iconColor={iconColor || "#2B2E72"} style={style} align={align} width={width} />
      <div>
        <Title title={title} titleSize={titleSize} align={align} padding={padding} />
        <Description description={description} size={size? size :"18px"} align={align} desWidth={desWidth} color={color} padding={padding} />
      </div>
