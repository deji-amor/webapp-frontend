...
const HardWareComponentType = () => {
	...
	useEffect(() => {
		if(hardwareComponentTypeList.every(item => isHardwareTypeValid(item))){
			dispatch(
				createTicketActions.updateField({ key: "hardwareComponentTypeListIsValid", value: true })
			);
		}else {
			dispatch(
				createTicketActions.updateField({ key: "hardwareComponentTypeListIsValid", value: false })
			);
		}
	}, [hardwareComponentTypeList]);
